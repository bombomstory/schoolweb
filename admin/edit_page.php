...
if(!empty($_POST)){
	if(strcmp($_POST["submit"],"&#10004; บันทึก")){		
		$sql="update ".$CFG->prefix."contents set title='".$_POST["title"]."',header='".$_POST["header"]."',detail='".$_POST["detail"]."' where id=".$_POST["eid"];
		$rs=mysqli_query($conn,$sql);
?>		
<div class="alert alert-success">
  <a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>
  <strong>บันทึกการแก้ไขข้อมูลเสร็จเรียบร้อยแล้ว!</strong> <a href="index.php">คลิกที่นี่ เพื่อดำเนินการต่อ...</a>
</div>
<?php		
	}
}else{
...
?>
		...
		เรื่อง: <input type="text" name="title" value ="<?=$data["title"];?>" size="80"><br />
